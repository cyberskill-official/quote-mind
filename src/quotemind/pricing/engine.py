"""Deterministic pricing (TASK-050..054).
...
def unit_price(
    ...
    """TASK-051 tiered unit price. A missing dealer price falls back to the list price."""
...
def line_total(
    ...
    """TASK-050 line subtotal after discount, whole đồng."""
...
def blended_margin(pairs: list[tuple[Decimal | int, Decimal | int]]) -> Decimal:
    """TASK-053 revenue-weighted blended margin over (sell, cost) pairs. Zero revenue yields 0."""
    total_sell = ZERO
    total_cost = ZERO
...
def quote_totals(lines: list[QuoteLine]) -> Totals:
    """TASK-050 aggregate subtotal, per-rate VAT breakdown, and grand total."""
    subtotal = ZERO
