...
def create(domain: str) -> None:
    ...
    body = fc.CreateCustomDomainInput(
        domain_name=domain,
        protocol="HTTP,HTTPS",  # HTTP first so the CNAME can be verified before a cert exists
        route_config=route,
    )

    try:
        client.create_custom_domain(fc.CreateCustomDomainRequest(body=body))
        print(f"created the custom domain: {domain}")
        ...
        if "DomainNameAlreadyExists" in message or "already exist" in message.lower():
            client.update_custom_domain(
                ...
                fc.UpdateCustomDomainRequest(
                    body=fc.UpdateCustomDomainInput(protocol="HTTP,HTTPS", route_config=route)
                ),
            )
            print(f"the custom domain already existed; its route was updated: {domain}")
        else:
            print(f"\nFC refused to create the domain:\n  {message}\n")
            print("If it says the domain is not verified, that is expected: add the CNAME below")
            print("first, wait for it to resolve, then run this again.")
